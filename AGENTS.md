...
- **Install & run** — `make clone-api` if `../trustbuddy-api` is missing, then `cp .env.example .env`, `make install`, and `make run` (Vite on `http://localhost:5173`). Sibling [trustbuddy-api](https://github.com/aegre/trustbuddy-api) at `../trustbuddy-api`: Docker full stack `make stack-up` or host `make run-dev` (API on `http://localhost:8080`). From this repo, `make stack-all-up` starts the API stack when the sibling is present, then the frontend containers (`http://localhost:3000`). Ensure API `CORS_ALLOWED_ORIGINS` includes `http://localhost:5173` and/or `http://localhost:3000`. Details: [README.md](README.md), [ARCHITECTURE.md](ARCHITECTURE.md).
